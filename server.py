...
logging.basicConfig(filename='Opvpn.log', level=logging.DEBUG)
...
while True :
	try:
		...
		logging.getLogger().info("receiving host port : %s",host_port)
		payload = payloadformating(getpayload(conf()),host_port)
		print(f"sending payload : {payload.encode()}")
		sock2.connect((proxy))
		payloadSend(payload,sock2)
		tunneling(sock2,cl)
		sock2.close()
		cl.close()
	except Exception as e:
		logging.getLogger().error("%s", e.args)
		break
